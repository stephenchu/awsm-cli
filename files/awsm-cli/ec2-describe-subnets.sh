...
Usage: ec2-describe-subnets [-r <region>...] [options]

Options:
    -r --region=<region>...                AWS region(s) in which the subnet(s) are in [required: as argument or stdin]
    -i --subnet-id=<subnet-id>...          AWS subnet-id(s) to describe
    -v --vpc-id=<vpc-id>...                AWS vpc-id(s) for the subnet(s)
...
aws:filters() {
  ...
  [ -z "$(string.join "," <<< "$subnet_ids")" ]  || filter_values="$filter_values Name=subnet-id,Values=$(string.join "," <<< "$subnet_ids")"
...
EOS
  ...
  jq -L $DIR/jq -r --arg region $region "include \"aws\"; .Subnets[] | ${jq:-$default}"
}
